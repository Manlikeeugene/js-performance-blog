...
//   const [likes, setLikes] = useState(dummyPost?.likes || 0);
...
//         setPost(fetchedPost);
//         const fetchedRelatedPosts = await fetchRelatedPosts(fetchedPost.category, id);
//         setRelatedPosts(fetchedRelatedPosts.length > 0 ? fetchedRelatedPosts : dummyRelatedPosts);
//         setError(null);
//       } catch (err) {
//         setPost(dummyPost);
//         setRelatedPosts(dummyRelatedPosts);
//         setError('Failed to load post. Showing demo data.');
//       } finally {
//         setLoading(false);
//       }
//     }
...
//     fetchData();
//   }, [id, baseUrl, dummyPost, dummyRelatedPosts]);
...
// //       // Optionally update post views in state if returned in response
...
//   // Fetch post, related posts, likes, and comments
//   useEffect(() => {
//     async function fetchPost() {
//       try {
//         const res = await fetch(`/api/posts/${id}`, {
...
//     //   setLoading(true);
//     //   try {
//     //     const [fetchedPost, fetchedLikes, fetchedComments] = await Promise.all([
//     //       fetchPost(),
//     //       fetchLikes(),
//     //       fetchComments(),
//     //     ]);
...
//       setLoading(true);
//       try {
//         const [postRes, likesRes, commentsRes] = await Promise.allSettled([
//           fetchPost(),
//           fetchLikes(),
//           fetchComments(),
//         ]);
...
//   // Copy buttons for code blocks (unchanged)
//   useEffect(() => {
//     const pres = document.querySelectorAll('pre');
//     pres.forEach(pre => {
//       if (!pre.querySelector('button.copy-btn')) {
...
//       // setSuccessMessage(data.liked ? 'Liked!' : 'Unliked!');
...
//       }
//       // setSuccessMessage('Link shared/copied!');
...
//   const handleSave = () => {
//     // Placeholder for future /api/save or /api/bookmarks endpoint
//     setSuccessMessage('Saved! (Feature coming soon)');
//   };
...
//                 <span className="text-sm text-slate-300">{post?.author || 'Unknown Author'}</span>
//               </div>
//               <div className="flex items-center gap-2">
//                 <Calendar className="w-4 h-4" />
//                 <span className="text-sm">
...
//                   <h3 className="text-lg sm:text-xl font-bold mb-2">{post?.author || 'Unknown Author'}</h3>
//                   <p className="text-slate-400 mb-4 text-sm leading-relaxed">{post?.authorBio || 'Author bio not available'}</p>
//                   <button className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 rounded-lg transition font-medium text-sm">
//                     Follow
//                   </button>
//                 </div>
//               </div>
...
//             <p className="text-slate-400 mb-6">Please log in to like or comment on posts.</p>
