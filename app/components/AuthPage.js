...
export default function AuthPage({ mode = 'login' }) {
  ...
  useEffect(() => {
    if (status === 'loading') return;
    if (session) {
      console.log('AuthPage: Session found, redirecting to /dashboard');
      router.push('/dashboard');
    }
  }, [session, status, router]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm() && !isSubmitting) {
      ...
      try {
        if (isLogin) {
          console.log('AuthPage: Attempting login with email:', formData.email);
          const result = await signIn('credentials', {
            email: formData.email,
            password: formData.password,
            redirect: false,
          ...
          console.log('AuthPage: Attempting signup with email:', formData.email);
          const response = await fetch('/api/auth/signup', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
          ...
          console.log('AuthPage: Signup response status:', response.status);

          if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Unknown error' }));
            setErrorMessage(errorData.message || 'Signup failed');
            console.error('AuthPage: Signup error:', errorData);
            return;
          }
        ...
        console.error('AuthPage: Network error:', error);
        setErrorMessage('Network error occurred. Please check your connection.');
      } finally {
        setIsSubmitting(false);
      }
    ...
    <div className="w-full max-w-md relative z-10 mt-20">
      <div className="bg-slate-900/50 backdrop-blur-xl rounded-3xl border border-slate-800/50 p-8 sm:p-10 shadow-2xl">
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-sm sm:text-base text-slate-400 px-2">
            {isLogin ? 'Sign in to continue to your account' : 'Join our community of developers'}
          </p>
        </div>
